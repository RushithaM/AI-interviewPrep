...
const CompanyQA = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      if (user && user.id) {
        try {
          const response = await authenticatedApi.getQuestions(user.id, 'company');
          if (response.success) {
            setQuestions(response.questions);
            const initialAnsweredCount = response.questions.filter(q => q.answer).length;
            setAnsweredCount(initialAnsweredCount);
            localStorage.setItem('companyAnsweredCount', initialAnsweredCount.toString());
          } else {
  ...
  const handleAnswerGenerated = () => {
    ...
    localStorage.setItem('companyAnsweredCount', newCount.toString());
    window.dispatchEvent(new Event('companyAnsweredCountUpdated'));
  };
    ...
    <Layout>
      ...
      >
        <div className="flex items-center justify-center mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-center text-gray-800 dark:text-white">Company Based Questions</h2>
        </div>
